pub mod snapshot_fuzz {
    ...
    pub fn fuzz_snapshot_import(data: &[u8]) {
        ...
        if data.len() >= 80 { state_root.copy_from_slice(&data[48..80]); }
        ...
        let context = SnapshotExecutionContext {
            genesis_root, current_position: position, current_epoch: position.epoch,
            epoch_seal_hash: Some([0u8; 32]), execution_version: 1, sealed_epochs: vec![],
        };
    ...
    pub fn fuzz_resource_registry(data: &[u8]) {
        ...
        for _ in 0..num_ops {
            ...
            if last_id.is_none() || (data[offset.wrapping_sub(1)].is_multiple_of(2)) {
                ...
                if !matches!(parent.state, ResourceState::Active) { continue; }
                ...
                let child_meta = ResourceMetadata {
                    ...
                    archetype: ResourceArchetype::Asset,
                    state: ResourceState::Active,
                    lineage: ResourceLineage::single_ancestor(resource_id, parent_id, parent_hash, version),
                    contract_id: [1u8; 32],
                    owner: [2u8; 32],
                };
