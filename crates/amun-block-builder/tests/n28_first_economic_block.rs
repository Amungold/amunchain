...
fn n28_multiple_transfers_in_block() {
    ...
    let mut mempool = Mempool::new();
    let mut tx1 = Transaction { version:1, sender:a1, nonce:1, payload:TransactionPayload::Transfer(TransferPayload{to:a3,amount:200}), signature:vec![] };
    tx1.sign(&SigningKey::from_bytes(&s1)); mempool.add_transaction(tx1).unwrap();
    let mut tx2 = Transaction { version:1, sender:a2, nonce:1, payload:TransactionPayload::Transfer(TransferPayload{to:a3,amount:150}), signature:vec![] };
    tx2.sign(&SigningKey::from_bytes(&s2)); mempool.add_transaction(tx2).unwrap();
...
fn n28_failed_transaction_in_block() {
    ...
    let mut mempool = Mempool::new();
    let mut tx = Transaction { version:1, sender:alice, nonce:1, payload:TransactionPayload::Transfer(TransferPayload{to:[2u8;32],amount:500}), signature:vec![] };
    tx.sign(&SigningKey::from_bytes(&seed)); mempool.add_transaction(tx).unwrap();
    let block = builder.build_block(1, [0u8;32], &mut mempool, 100, [9u8;32], 1000);
    assert!(!block.receipts[0].success);
