use amun_constitutional_proof::{
    ConstitutionalReport, ConstitutionalVerdict, EvidenceArchive, EvidenceRecord, EvidenceStatus,
    EvidenceType, ObligationId, ObligationKind, ObligationNamespace, ObligationRegistry,
    ObligationResult, ObligationResultStatus, ObligationSeverity, ProofObligation,
    PublicationPackage, ReportGenerator, VerdictEvaluator,
};
...
impl ConstitutionalBridge {
    ...
    > {
        ...
        for (phase, cert) in &phase_certificates {
            ...
            let obligations: Vec<ProofObligation> = registry
                .all_obligations()
        ...
        for phase in &["N41", "N42", "N43", "N44", "N45", "N46"] {
            ...
            for obl in &obligations {
                ...
                let status = if ev.is_empty() {
                    ObligationResultStatus::Inconclusive
                } else {
                    ObligationResultStatus::Satisfied
                };
        ...
        let report =
            ReportGenerator::generate_report(&registry, &archive, verdicts.clone(), timestamp);
...
mod tests {
    use super::*;
    use amun_verification_kernel::{
        ClaimStatus, ClaimType, ConstitutionalClaim, Evidence as VKEvidence,
        EvidenceType as VKEvidenceType,
    };
    ...
    fn make_cert(phase: &str) -> VerificationCertificate {
        VerificationCertificate::issue(
            ...
            vec![ConstitutionalClaim {
                claim_id: format!("{}-C1", phase),
                claim_type: ClaimType::Safety,
                description: format!("Claim {}", phase),
    ...
    fn n47_7_run_full_pipeline() {
        ...
        for p in &["N41", "N42", "N43", "N44", "N45", "N46"] {
            certs.insert(p.to_string(), make_cert(p));
        }
