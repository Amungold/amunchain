...
// ============================================================================
// BootstrapContext (final, single version)
// ============================================================================

pub struct BootstrapContext {
    pub config: AmunConfig,
...
fn load_certificate(path: &Path) -> Result<ValidatorCertificate, BootstrapError> {
    let content = std::fs::read_to_string(path)
        .map_err(|e| BootstrapError::Certificate(format!("Cannot read certificate: {}", e)))?;
    let cert: ValidatorCertificate = serde_json::from_str(&content)
        .map_err(|e| BootstrapError::Certificate(format!("Invalid certificate JSON: {}", e)))?;
    Ok(cert)
}
...
// ============================================================================
// Single public bootstrap function
// ============================================================================

pub fn bootstrap(config: AmunConfig) -> Result<BootstrapContext, BootstrapError> {
...
impl From<amun_live_cluster::config::ValidatorConfig> for AmunConfig {
    fn from(config: amun_live_cluster::config::ValidatorConfig) -> Self {
        AmunConfig {
            validator_id: config.validator_id,
            key_file: PathBuf::from(format!("{}/key.bin", config.data_dir)),
            certificate_path: None,
            listen_addr: config.listen_addr,
            peer_addresses: config.other_peers().iter().map(|p| p.address).collect(),
            data_dir: PathBuf::from(&config.data_dir),
            genesis_file: PathBuf::from("genesis.bin"),
            quorum_size: config.quorum_size.unwrap_or(3),
        }
    }
}
...
mod tests {
    ...
    fn test_load_or_create_keypair() {
        let tmp = std::env::temp_dir().join("test_kp.bin");
        let _ = std::fs::remove_file(&tmp);
        let kp = load_or_create_keypair(&tmp).unwrap();
        assert!(tmp.exists());
        let kp2 = load_or_create_keypair(&tmp).unwrap();
        assert_eq!(kp.peer_id().0, kp2.peer_id().0);
        let _ = std::fs::remove_file(&tmp);
    }

    #[test]
    fn test_certificate_mismatched_validator_rejected() {
        let cert = ValidatorCertificate {
            validator_id: PeerId([0xBB; 32]),
            public_key: [0xCC; 32],
