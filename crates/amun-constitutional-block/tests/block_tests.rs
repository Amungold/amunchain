use amun_constitutional_block::{Blockchain, ConstitutionalBlock};
...
fn test_block_hash_deterministic() {
    let b1 = ConstitutionalBlock::new(
        0,
        "0".repeat(64),
        "t".into(),
        "p".into(),
    ...
    let b2 = ConstitutionalBlock::new(
        0,
        "0".repeat(64),
        "t".into(),
        "p".into(),
...
fn test_verify_block_provenance_wrong_certificate_fails() {
    ...
    let wrong_root = hex::encode(ConstitutionalStateRuntime::certificate_merkle_root(&[]));
...
fn n8_light_client_wrong_certificate_fails() {
    ...
    let inclusion_proof =
        ConstitutionalStateRuntime::prove_certificate_inclusion(&certs, &hash1).unwrap();
