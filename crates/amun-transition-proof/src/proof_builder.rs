...
use crate::enhanced_proof::{EnhancedTransitionProof, LineageProof, MerkleProof, WitnessBundle};
...
impl ProofBuilder {
    pub fn build(
        ...
        let evidence: Vec<ConstitutionalEvidence> = buffer
            .all_evidence()
            .iter()
            .map(|ev| EvidenceEngine::convert(ev, contract_id, block_height, transaction_hash))
            .collect();
        TransitionProof::new(
            transaction_hash,
            contract_id,
    ...
    fn compute_siblings(registry: &ResourceRegistry, rid: &ResourceId) -> Vec<([u8; 32], bool)> {
        ...
        let leaves: Vec<([u8; 32], ResourceId)> = ids
            .iter()
            .filter_map(|id| {
                registry
                    .get(id)
                    .map(|m| (ResourceRegistry::hash_resource(m), *id))
            })
            .collect();
        ...
        while level.len() > 1 {
            ...
            if sib < level.len() {
                siblings.push((level[sib], !is_left));
            }
    ...
    fn build_lineage_proof(registry: &ResourceRegistry, rid: &ResourceId) -> LineageProof {
        ...
        while let Some(m) = registry.get(&cur) {
            if m.lineage.parent_resource_ids.is_empty() {
                gen = cur;
                break;
            }
            let p = m.lineage.parent_resource_ids[0];
            let h = if m.lineage.parent_hashes.len() == 1 {
                m.lineage.parent_hashes[0]
            } else {
                [0u8; 32]
            };
            chain.push((p, h));
    ...
    fn compute_enhanced_hash(proof: &EnhancedTransitionProof) -> [u8; 32] {
        ...
        h.update(&proof.transaction_hash);
        h.update(proof.contract_id.as_bytes());
        h.update(&proof.block_height.to_le_bytes());
        h.update(&proof.block_hash);
        h.update(&proof.pre_state_root);
        h.update(&proof.post_state_root);
        h.update(&proof.gas_used.to_le_bytes());
        for id in &proof.consumed_resources {
            h.update(id.as_bytes());
        }
        for m in &proof.produced_resources {
            h.update(m.resource_id.as_bytes());
            h.update(&m.lineage.version.to_le_bytes());
        }
        for ev in &proof.evidence {
            h.update(&ev.evidence_id());
        }
