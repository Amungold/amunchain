use crate::types::{BlockProposal, Vote, QuorumCertificate};
...
impl ActionRecord {
    pub fn compute_hash(&self) -> [u8; 32] {
        ...
        for (i, b) in self.height.to_le_bytes().iter().enumerate() { state[i] ^= b; }
        for (i, b) in self.round.to_le_bytes().iter().enumerate() { state[i] ^= b; }
        for (i, b) in self.validator_id.iter().enumerate() { state[i] ^= b; }
        if let Some(prev) = &self.prev_hash {
            for i in 0..32 { state[i] ^= prev[i]; }
        }
        state
    }
}
...
impl ActionLog {
    pub fn record(&mut self, action: ConsensusAction, height: u64, round: u64, step: &str, validator_id: [u8; 32]) {
        let prev_hash = self.records.last().map(|r| r.hash);
        let mut record = ActionRecord { action, height, round, step: step.to_string(), validator_id, prev_hash, hash: [0u8; 32] };
    ...
    pub fn verify(&self) -> bool {
        for i in 0..self.records.len() {
            let expected_prev = if i > 0 { Some(self.records[i - 1].hash) } else { None };
            if self.records[i].prev_hash != expected_prev { return false; }
            if self.records[i].hash != self.records[i].compute_hash() { return false; }
        }
        true
    }

    pub fn evidence_root(&self) -> [u8; 32] {
        let mut tree = amun_constitutional_commitments::SparseMerkleTree::new(b"AMUN_EVIDENCE_DOMAIN");
    ...
    pub fn prove(&self, index: usize) -> Option<EvidenceProof> {
        ...
        Some(EvidenceProof { action_hash: record.hash, proof, evidence_root: tree.root() })
    }
}
