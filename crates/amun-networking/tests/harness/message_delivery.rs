...
impl Default for DeliveryPolicy {
    fn default() -> Self {
        Self { loss_rate: 0.0, base_delay_ms: 0, jitter_ms: 0 }
    }
}
...
impl MessageDeliveryEngine {
    pub fn new(policy: DeliveryPolicy, seed: u64) -> Self {
        Self { pending: Vec::new(), policy, rng: StdRng::seed_from_u64(seed) }
    }

    pub fn schedule(&mut self, delivery_time: u64, recipient: String, envelope: Envelope) {
        self.pending.push(DelayedEnvelope { delivery_time, recipient, envelope });
    }
    ...
    pub fn broadcast(
        ...
        for recipient_id in node_ids {
            if recipient_id == sender_id && !include_self { continue; }
            if self.policy.loss_rate > 0.0 && self.rng.gen::<f64>() < self.policy.loss_rate { continue; }
            self.schedule(delivery_time, recipient_id.clone(), envelope.clone()); eprintln!("  SCHEDULED to={} at={}", recipient_id, delivery_time);
        }
        delivery_time
    }
