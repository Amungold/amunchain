...
struct MultiRoundScenario { rounds: u64 }
...
impl ConsensusScenario for MultiRoundScenario {
    ...
    fn schedule_events(&self, s: &mut EventScheduler, nids: &[String]) {
        ...
        s.schedule(self.rounds * 50 + 10, nids[0].clone(), EventType::CheckTimeout);
    }
}
...
fn test_multi_round_consensus() {
    for &rounds in &[1, 2, 3, 5] {
        ...
        assert!(result.success, "Multi-round failed at {} rounds: {} commits", rounds, result.commits);
    }
}
