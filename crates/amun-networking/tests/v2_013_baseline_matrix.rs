...
struct BaselineScenario { delay_ms: u64, loss_rate: f64 }

impl BaselineScenario {
    fn new(delay_ms: u64, loss_rate: f64) -> Self { Self { delay_ms, loss_rate } }
}
...
impl ConsensusScenario for BaselineScenario {
    ...
    fn config(&self) -> ScenarioConfig {
        ScenarioConfig {
            delay_ms: self.delay_ms, loss_rate: self.loss_rate,
            jitter_ms: 0, proposal_retries: 3,
...
fn test_baseline_matrix() {
    ...
    for (delay_ms, loss_rate) in conditions {
        ...
        let success_rate = committed_counts.iter().filter(|&&c| c >= 27).count() as f64 / trials as f64 * 100.0;

        println!("{:>5}ms | {:>4}% | {:>3} | {:>3} | {:>4.1} | {:>11.1}%",
                 delay_ms, (loss_rate * 100.0) as u32, min, max, avg, success_rate);
    }
}
