    ...
    pub fn recover(snapshot: &StateSnapshot, wal: &WriteAheadLog) -> AmunResult<PersistentStore> {
        if !wal.verify_chain_integrity() {
            return Err(FailureContext::new(
                ConstitutionalFault::JournalHashMismatch,
                0x000B,
                0x0050,
            ));
        }
        ...
        for (k, v) in &snapshot.entries {
            store.apply_replay(&WalPayload::Set {
                key: {
                    let mut kc: Vec<u8, 32> = Vec::new();
                    kc.extend_from_slice(k).map_err(|_| {
                        FailureContext::new(ConstitutionalFault::CapacityExceeded, 0x000B, 0x0040)
                    })?;
                    kc
                },
                value: {
                    let mut vc: Vec<u8, 64> = Vec::new();
                    vc.extend_from_slice(v).map_err(|_| {
                        FailureContext::new(ConstitutionalFault::CapacityExceeded, 0x000B, 0x0041)
                    })?;
                    vc
                },
            })?;
