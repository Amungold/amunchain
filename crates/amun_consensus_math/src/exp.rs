...
use crate::constants::{
    EXP_DOMAIN_MIN, EXP_DOMAIN_MAX, MAX_FIXED, MIN_FIXED, SCALE
};
...
mod tests {
    ...
    fn test_exp_half() {
        ...
        let result = f_exp(x);
        // exp(0.5) ≈ 1.648721
        assert!(result.raw() > 1_640_000, "exp(0.5) = {} too low", result.raw());
        assert!(result.raw() < 1_660_000, "exp(0.5) = {} too high", result.raw());
    }
    ...
    fn test_exp_one() {
        ...
        let result = f_exp(x);
        // exp(1) ≈ 2.718282
        assert!(result.raw() > 2_710_000, "exp(1) = {} too low", result.raw());
        assert!(result.raw() < 2_730_000, "exp(1) = {} too high", result.raw());
    }
    ...
    fn test_exp_negative_one() {
        ...
        assert!(result.raw() < 368_000, "exp(-1) = {} too high", result.raw());
    }
    ...
    fn test_exp_negative_half() {
        ...
        let result = f_exp(x);
        // exp(-0.5) ≈ 0.606531
        assert!(result.raw() > 606_000, "exp(-0.5) = {} too low", result.raw());
        assert!(result.raw() < 607_000, "exp(-0.5) = {} too high", result.raw());
    }
    ...
    fn test_exp_two() {
        ...
        let result = f_exp(x);
        // exp(2) ≈ 7.389056
        assert!(result.raw() > 7_380_000, "exp(2) = {} too low", result.raw());
        assert!(result.raw() < 7_400_000, "exp(2) = {} too high", result.raw());
    }
}
