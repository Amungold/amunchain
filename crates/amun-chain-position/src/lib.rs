...
impl EpochSeal {
    pub fn new(
        ...
        h.update(&epoch_root);
        h.update(&journal_root);
        h.update(&replay_root);
    ...
    pub fn verify(&self) -> bool {
        ...
        h.update(&self.epoch_root);
        h.update(&self.journal_root);
        h.update(&self.replay_root);
        let mut c = [0u8; 32];
        c.copy_from_slice(&h.finalize().as_bytes()[..32]);
        c == self.seal_hash
    }
}
...
impl ChainPosition {
    ...
    pub fn next_sequence(&self) -> Option<Self> {
        self.sequence.checked_add(1).map(|s| Self {
            epoch: self.epoch,
            sequence: s,
        })
    }
    pub fn next_epoch(&self) -> Option<Self> {
        self.epoch.checked_add(1).map(|e| Self {
            epoch: e,
            sequence: 0,
        })
    }
    pub fn is_genesis(&self) -> bool {
        self.epoch == 0 && self.sequence == 0
    }
    pub fn hash(&self) -> [u8; 32] {
        let mut h = Hasher::new();
        h.update(b"AMUN_CHAIN_POS_V1");
        h.update(&self.epoch.to_le_bytes());
