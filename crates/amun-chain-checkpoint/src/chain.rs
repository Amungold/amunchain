...
use crate::inclusion::{CheckpointBundle, prove_checkpoint_inclusion, checkpoint_merkle_root};
...
impl RecursiveCheckpointProof {
    ...
    pub fn from_checkpoints(checkpoints: &[CheckpointCertificate]) -> Option<Self> {
        ...
        Some(Self { bundles, common_root: root })
    }
}
...
mod tests {
    ...
    #[test] fn n13_single_checkpoint_chain() { assert!(RecursiveCheckpointProof::from_checkpoints(&[create_checkpoint(0,0)]).unwrap().verify().is_ok()); }
    #[test] fn n13_two_checkpoint_chain() { let c1 = create_checkpoint(0,1); let c2 = create_checkpoint(2,3); assert!(RecursiveCheckpointProof::from_checkpoints(&[c1,c2]).unwrap().verify().is_ok()); }
    #[test] fn n13_discontinuous_chain_fails() { let c1 = create_checkpoint(0,0); let c2 = create_checkpoint(2,2); assert!(RecursiveCheckpointProof::from_checkpoints(&[c1,c2]).unwrap().verify().is_err()); }
    #[test] fn n13_wrong_root_fails() { let mut proof = RecursiveCheckpointProof::from_checkpoints(&[create_checkpoint(0,0)]).unwrap(); proof.common_root = [0xFF;32]; assert!(proof.verify().is_err()); }
}
