...
use amun_constitutional_commitments::smt::{MerkleProof, SparseMerkleTree};
...
pub fn prove_checkpoint_inclusion(
    ...
    let index = checkpoints
        .iter()
        .position(|c| c.checkpoint_hash_bytes() == *checkpoint_hash)?;
    let mut tree = SparseMerkleTree::new(CHECKPOINT_MERKLE_DOMAIN);
    for (i, cert) in checkpoints.iter().enumerate() {
        let key = format!("{:020}", i);
        tree.insert(key.as_bytes(), &cert.checkpoint_hash_bytes());
...
mod tests {
    ...
    #[test]
    fn n12b_single_checkpoint_merkle_root() {
        let c = create_checkpoint(0, 0);
        assert_ne!(checkpoint_merkle_root(&[c]), [0u8; 32]);
    }
    #[test]
    fn n12b_multiple_checkpoints_different_root() {
        let c1 = create_checkpoint(0, 0);
        let c2 = create_checkpoint(1, 1);
        assert_ne!(
            checkpoint_merkle_root(std::slice::from_ref(&c1)),
            checkpoint_merkle_root(&[c1, c2])
        );
    }
    #[test]
    fn n12b_inclusion_proof_valid() {
        let c1 = create_checkpoint(0, 0);
        let c2 = create_checkpoint(1, 1);
        let certs = vec![c1.clone(), c2.clone()];
        let root = checkpoint_merkle_root(&certs);
        let proof = prove_checkpoint_inclusion(&certs, &c1.checkpoint_hash_bytes()).unwrap();
        assert_eq!(proof.root, root);
        assert!(proof.verify());
    }
    #[test]
    fn n12b_inclusion_proof_wrong_hash_fails() {
        let c1 = create_checkpoint(0, 0);
        let c2 = create_checkpoint(1, 1);
        let certs = vec![c1.clone(), c2.clone()];
        let mut proof = prove_checkpoint_inclusion(&certs, &c1.checkpoint_hash_bytes()).unwrap();
        proof.checkpoint_hash = c2.checkpoint_hash_bytes();
        assert!(!proof.verify());
    }
    #[test]
    fn n12b_root_mismatch_fails() {
        let c1 = create_checkpoint(0, 0);
        let certs = vec![c1.clone()];
        let mut proof = prove_checkpoint_inclusion(&certs, &c1.checkpoint_hash_bytes()).unwrap();
        proof.root = [0xFF; 32];
        assert!(!proof.verify());
    }
    #[test]
    fn n12c_bundle_valid() {
        let c = create_checkpoint(0, 0);
        let certs = vec![c.clone()];
        let proof = prove_checkpoint_inclusion(&certs, &c.checkpoint_hash_bytes()).unwrap();
        assert!(CheckpointBundle::new(c, proof).verify().is_ok());
    }
    #[test]
    fn n12d_light_verify_sequence() {
        let c1 = create_checkpoint(0, 0);
        let c2 = create_checkpoint(1, 1);
        let certs = vec![c1.clone(), c2.clone()];
        let root = checkpoint_merkle_root(&certs);
        let p1 = prove_checkpoint_inclusion(&certs, &c1.checkpoint_hash_bytes()).unwrap();
        let p2 = prove_checkpoint_inclusion(&certs, &c2.checkpoint_hash_bytes()).unwrap();
        let bundles = vec![CheckpointBundle::new(c1, p1), CheckpointBundle::new(c2, p2)];
        assert!(verify_checkpoint_sequence(&bundles, &root).is_ok());
    }
    #[test]
    fn n12d_light_verify_wrong_root() {
        let c1 = create_checkpoint(0, 0);
        let certs = vec![c1.clone()];
        let p1 = prove_checkpoint_inclusion(&certs, &c1.checkpoint_hash_bytes()).unwrap();
        assert!(verify_checkpoint_sequence(&[CheckpointBundle::new(c1, p1)], &[0x11; 32]).is_err());
    }
}
