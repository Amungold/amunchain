...
fn build_state() -> (ResourceRegistry, RoyaltyLedger, GovernanceLedger, BridgeLedger, ConstitutionalRegistry, [u8; 32], [u8; 32], [u8; 32], [u8; 32], [u8; 32]) {
    ...
    for i in 1..6u8 {
        ...
        const_reg.register(NftConstitutionalRecord {
            token_id: token, owner, collection_id: Some(col_id.0), creator: owner,
            mining_origin: Some("Test".into()), royalty_policy: None, governance_right: None, bridge_lock: None,
        });
    }
    ...
    (reg, royalty, gov, bridge, const_reg, state_root, royalty_root, gov_root, bridge_root, const_root)
}
...
fn n151_root_changes_after_mutation() {
    let (mut reg, mut royalty, mut gov, _bridge, mut const_reg, sr_before, rr_before, gr_before, _br_before, cr_before) = build_state();
    ...
    reg.derive_from_collection(&col_id, ResourceMetadata {
        ...
        archetype: ResourceArchetype::NFTAsset,
        state: ResourceState::Active,
        lineage: ResourceLineage::single_ancestor(ResourceId(new_token), col_id, parent_hash, version),
        contract_id: [0u8; 32],
        owner: new_owner,
    }).unwrap();

    royalty.settle(&amun_nft_royalty::RoyaltyRecord {
        token_id: new_token, creator: new_owner, payer: [0u8; 32], sale_price: 200, royalty_amount: 20, block_height: 99,
    });

    gov.set_rights(amun_nft_governance::GovernanceRight {
        token_id: new_token, owner: new_owner, can_propose: true, can_veto: true, voting_power: 99,
    });

    const_reg.register(NftConstitutionalRecord {
        token_id: new_token, owner: new_owner, collection_id: Some(col_id.0), creator: new_owner,
        mining_origin: Some("Mutation".into()), royalty_policy: None, governance_right: None, bridge_lock: None,
    });
