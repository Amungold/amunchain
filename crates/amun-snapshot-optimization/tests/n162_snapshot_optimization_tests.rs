...
fn n162_incremental_snapshot_faster_than_full() {
    ...
    println!("Full snapshot: {}ms, Incremental delta count: {}, query: {}ms", full_snapshot_ms, delta_count, delta_ms);
    assert!(delta_count > 0);
    assert!(delta_ms <= full_snapshot_ms + 2, "Delta query should not be significantly slower");
}
...
fn n162_restore_from_compressed_is_fast() {
    ...
    println!("Compressed count: {}, Restored active: {}, Time: {}ms", compressed.len(), restored.total_active(), restore_ms);
    assert!(compressed.len() > 0, "Compressed snapshot should have resources");
    assert!(restore_ms < 50, "Restore from compressed should be fast, got {}ms", restore_ms);
}
