...
fn test_round_encoding() {
    ...
    let decoded = ConsensusRound::decode_exact(&buf).expect("test invariant");
    assert_eq!(decoded, r);
}
...
fn test_bitmap_set_get() {
    ...
    bm.set(0).expect("test invariant");
    assert!(bm.get(0));
}
...
fn test_message_roundtrip() {
    ...
    let len = msg.encode(&mut buf).expect("test invariant");
    let decoded = ConsensusMessage::decode_exact(&buf[..len]).expect("test invariant");
