...
mod tests {
    ...
    fn n105_sign_and_verify_vote() {
        ...
        let payload = vote_signing_payload(&voter_id, 42, &[0xAA; 32], &[0xBB; 32], true, 1000);
    ...
    fn n105_tampered_payload_rejected() {
        let (sk, pk) = gen_key();
        let voter_id = crate::validator_id::derive_validator_id(&pk);
        let mut payload = vote_signing_payload(&voter_id, 42, &[0xAA; 32], &[0xBB; 32], true, 1000);
        payload[20] ^= 1; // tamper
                          // Sign original payload (valid)
        let original_payload =
            vote_signing_payload(&voter_id, 42, &[0xAA; 32], &[0xBB; 32], true, 1000);
