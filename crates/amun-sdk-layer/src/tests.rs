...
mod tests {
    ...
    #[test]
    fn test_token_api_transfer() { let mut token = TokenApi::create_account(1_000_000).data.unwrap(); let result = TokenApi::transfer(&mut token, 100_000); assert!(result.success); }
    #[test]
    ...
    #[test]
    fn test_sandbox_simulation() { let mut sandbox = Sandbox::new(); let a0 = sandbox.create_account(1_000_000).data.unwrap(); let a1 = sandbox.create_account(500_000).data.unwrap(); let result = sandbox.simulate_transfer(a0, a1, 100_000); assert!(result.success); }
    #[test]
