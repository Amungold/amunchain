...
mod tests {
    ...
    fn test_header_roundtrip() {
        let h = hdr();
        let mut b = [0u8; 256];
        let l = h.encode(&mut b).unwrap();
        assert_eq!(
            BlockHeader::decode_exact(&b[..l]).unwrap().height,
            BlockHeight(1)
        );
    }
    #[test]
    ...
    fn test_body_rejects_exceeding_limit() {
        ...
        let mut b = BlockBody::new();
        b.add_tx_hash(PublicHash32::new([0x01u8; 32]), &l).unwrap();
        b.add_tx_hash(PublicHash32::new([0x02u8; 32]), &l).unwrap();
    ...
    fn test_block_roundtrip() {
        ...
        let len = blk.encode(&mut buf).unwrap();
        assert_eq!(Block::decode_exact(&buf[..len]).unwrap().tx_count(), 1);
    }
    #[test]
    ...
    fn test_block_body_wire_freeze() {
        ...
        body.add_tx_hash(PublicHash32::new([0x22u8; 32]), &limits)
            .unwrap();
        let mut buf = [0u8; 256];
        let _len = body.encode(&mut buf).unwrap();
        assert_eq!(_len, 68);
    ...
    fn test_block_body_wire_freeze_boundary() {
        ...
        body.add_tx_hash(PublicHash32::new([0x22u8; 32]), &limits)
            .unwrap();
        let mut buf = [0u8; 256];
        let _ = body.encode(&mut buf).unwrap();
