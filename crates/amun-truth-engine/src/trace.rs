...
impl StateTrace {
    pub fn new() -> Self {
        Self {
            entries: Vec::with_capacity(MAX_TRACE),
        }
    }
    pub fn append(
        &mut self,
        ...
        if self.entries.len() >= MAX_TRACE {
            return Err("trace full");
        }
        self.entries.push(TraceEntry {
            from_root: from,
            to_root: to,
            via_message_hash: hash,
        });
        Ok(())
    }
    pub fn find_transition(&self, from: [u8; 32], hash: [u8; 32]) -> Option<&TraceEntry> {
        self.entries
            .iter()
            .rev()
            .find(|e| e.from_root == from && e.via_message_hash == hash)
    }
