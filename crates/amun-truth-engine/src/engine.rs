use crate::log::{MessageLog, MessageEntry};
...
pub enum ReplayError {
    DivergenceAt { position: ChainPosition, expected: [u8; 32], actual: [u8; 32] },
    InvalidTransition { at_position: ChainPosition },
    ChunkReadError { position: ChainPosition },
    JournalContinuityBroken { at_position: ChainPosition },
    JournalAppendError { position: ChainPosition },
    PositionOverflow,
    EpochAlreadySealed { epoch: u64 },
    PositionMismatch { expected: ChainPosition, actual: ChainPosition },
}

impl TruthEngine {
    pub fn new(genesis_root: [u8; 32]) -> Self {
        Self {
            message_log: MessageLog::new(), state_trace: StateTrace::new(), genesis_root,
            live_state: StateMachine::new(genesis_root, EXECUTION_VERSION),
            live_journal: StorageJournal::new(genesis_root),
            current_position: ChainPosition::genesis(), epoch_seals: Vec::new(), sealed_epochs: BTreeSet::new(),
        }
    }

    fn prepare(&self, payload: &[u8]) -> Result<(ChainPosition, Vec<u8>), &'static str> {
        if self.sealed_epochs.contains(&self.current_position.epoch) { return Err("epoch sealed"); }
        let np = self.current_position.next_sequence().ok_or("overflow")?;
        Ok((np, payload.to_vec()))
    }

    fn commit_transition(
        &mut self, position: ChainPosition, payload: &[u8], output: TransitionOutput,
    ) -> Result<(), &'static str> {
        self.state_trace.append(output.receipt.from_root, output.new_root, output.receipt.hash())
            .map_err(|_| "trace failed")?;
        self.live_journal.append(output.receipt.clone()).map_err(|_| "journal failed")?;
        self.message_log.append(position, payload)?;
        self.current_position = position;
        self.live_state.apply_overlay(output.overlay, output.new_root);
        Ok(())
    }

    pub fn execute_live(&mut self, tx_data: &[u8], gas_limit: u64) -> Result<([u8; 32], ProtocolEvent), &'static str> {
        let (position, payload) = self.prepare(tx_data)?;
        let (receipt, overlay) = StateMachine::execute_transition(
            &self.live_state.state, self.live_state.execution_version,
            self.live_state.epoch_seal_hash, position, &payload, gas_limit,
        );
        let new_root = receipt.to_root;
        let output = TransitionOutput { receipt, overlay, new_root };
        self.commit_transition(position, &payload, output)?;
        let event = ProtocolEvent::ExecuteTransaction {
            position: self.current_position, payload: tx_data.to_vec(), expected_root: new_root,
        };
        Ok((new_root, event))
    }
    ...
    pub fn seal_and_advance_epoch(&mut self) -> Result<(EpochSeal, ProtocolEvent), &'static str> {
        ...
        let seal = EpochSeal::new(cur, self.live_state.live_root(), self.live_journal.chain_hash(),
            self.compute_chain_root_until(self.current_position).map_err(|_| "replay failed")?);
        ...
        let event = ProtocolEvent::SealEpoch {
            position: sp, epoch: cur, seal_hash: seal.seal_hash,
    ...
    pub fn apply_event(&mut self, event: &ProtocolEvent) -> Result<[u8; 32], &'static str> {
        let expected_next = self.current_position.next_sequence().ok_or("position overflow")?;
        if event.position() != expected_next { return Err("position mismatch"); }
        match event {
            ProtocolEvent::ExecuteTransaction { payload, expected_root, .. } => {
                let (position, payload) = self.prepare(payload)?;
                let (receipt, overlay) = StateMachine::execute_transition(
                    &self.live_state.state, self.live_state.execution_version,
                    self.live_state.epoch_seal_hash, position, &payload, 1_000_000,
                );
                let new_root = receipt.to_root;
                if new_root != *expected_root { return Err("execute: root mismatch"); }
                let output = TransitionOutput { receipt, overlay, new_root };
                self.commit_transition(position, &payload, output)?;
                Ok(new_root)
            }
            ProtocolEvent::SealEpoch { position, epoch, seal_hash, expected_root } => {
                let sp = *position;
                let seal = EpochSeal {
                    epoch: *epoch, seal_hash: *seal_hash,
                    epoch_root: self.live_state.live_root(),
                    journal_root: self.live_journal.chain_hash(),
                    replay_root: self.compute_chain_root_until(self.current_position).map_err(|_| "replay failed")?,
                };
                self.message_log.append_seal(sp, seal.seal_hash)?;
                self.live_state.set_epoch_seal(seal.seal_hash);
                ...
                self.current_position = sp.next_epoch().ok_or("overflow")?;
                self.live_journal.set_epoch(self.current_position.epoch);
                if self.live_state.live_root() != *expected_root { return Err("seal: root mismatch"); }
    ...
    pub fn export_snapshot(&self) -> Result<CanonicalSnapshot, amun_snapshot_constitution::SnapshotError> {
    ...
    pub fn compute_chain_root_until(&self, until: ChainPosition) -> Result<[u8; 32], ReplayError> {
        if until.is_genesis() { return Ok(self.genesis_root); }
        let mut sm = StateMachine::new(self.genesis_root, EXECUTION_VERSION);
        for entry in self.message_log.entries() {
            if entry.position() > until { break; }
            match &entry.transcript {
                crate::log::TranscriptEntry::EpochSeal { seal_hash, .. } => sm.set_epoch_seal(*seal_hash),
                crate::log::TranscriptEntry::Transaction { .. } => {
                    let p = self.message_log.payload(entry).ok_or(ReplayError::ChunkReadError { position: entry.position() })?;
    ...
    pub fn compute_chain_root(&self, target_tx_count: u64) -> Result<[u8; 32], ReplayError> {
        if target_tx_count == 0 { return Ok(self.genesis_root); }
        let mut sm = StateMachine::new(self.genesis_root, EXECUTION_VERSION);
        let mut tc: u64 = 0;
        for entry in self.message_log.entries() {
            match &entry.transcript {
    ...
    pub fn live_root(&self) -> [u8; 32] { self.live_state.live_root() }
    pub fn genesis_root(&self) -> [u8; 32] { self.genesis_root }
    pub fn current_position(&self) -> ChainPosition { self.current_position }
    pub fn epoch_seals(&self) -> &[EpochSeal] { &self.epoch_seals }
    pub fn live_journal(&self) -> &StorageJournal { &self.live_journal }
}
