...
use amun_state_sync::state_chunk::{StateChunk, build_chunk_merkle_tree};
...
impl SyncTransport {
    pub fn export_snapshot(
        ...
        let chunks: Vec<StateChunk> = active
            .chunks(chunk_size)
            .enumerate()
            .map(|(i, batch)| StateChunk::new(i as u32, batch.iter().map(|m| (*m).clone()).collect()))
            .collect();
        let (chunk_root, chunk_proofs) = build_chunk_merkle_tree(&chunks);
        let cert = StateSnapshotCertificate::new(
            height, block_hash, state_root, history_root, chunk_root,
            chunks.len() as u32, active.len() as u64,
            0, verifier,
        );
    ...
    pub fn import_snapshot(
        ...
        match result {
            SyncVerificationResult::Verified { state_root, resources_imported: _ } => {
