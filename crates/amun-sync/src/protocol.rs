pub use amun_network_transport::protocol::{
    MSG_VOTE, MSG_TIP_REQUEST, MSG_TIP_RESPONSE,
    MSG_BLOCK_RANGE_REQUEST, MSG_BLOCK_RANGE_RESPONSE,
    send_vote, send_tip_request, send_block_range_request, handle_incoming,
};
...
pub fn handle_incoming_with_store(
    ...
    match msg_type[0] {
        ...
        MSG_BLOCK_RANGE_REQUEST => {
            ...
            if stream.read_exact(&mut range_buf).is_err() { return; }
            ...
            eprintln!("SYNC_SERVER: store has {} records, highest={}", store.len(), store.latest_height());
            ...
            for r in &records {
                eprintln!("SYNC_SERVER_RECORD: height={} len={}", r.height, r.encode().len());
