...
pub fn download_missing_records(
    ...
    for attempt in 0..max_retries {
        eprintln!("CATCHUP_ATTEMPT {}", attempt);
        eprintln!("CATCHUP_ATTEMPT {}", attempt);
        eprintln!("DISCOVER_PEER");
        eprintln!("DISCOVER_PEER");
        if let Some(peer) = discover_peer_tip(peers) {
            eprintln!("PEER_FOUND height={}", peer.tip_height);
            eprintln!(
                "FOUND_PEER tip={} current={}",
                peer.tip_height, current_height
            );
            if peer.tip_height <= current_height {
                // We are already at the tip or ahead
                eprintln!("ALREADY_AT_TIP");
                return Ok(Vec::new());
            }
            ...
            eprintln!("BEFORE_DOWNLOAD");
            eprintln!("DOWNLOAD_RANGE {}..{}", start, end);
            match download_block_range(peer.address, start, end) {
                Ok(records) => {
                    eprintln!("DOWNLOADED {} RECORDS", records.len());
                    eprintln!("DOWNLOAD_OK {}", records.len());
                    eprintln!("SYNC: received {} blocks", records.len());
                    return Ok(records);
                }
                Err(e) => {
                    eprintln!("DOWNLOAD_ERR {}", e);
