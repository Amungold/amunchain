...
fn main() {
    ...
    let source_store =
        ChainStore::open(source_dir.to_str().unwrap()).expect("Failed to open source store");
    eprintln!(
        "Creating snapshot from {} at height {}...",
        source_dir.display(),
        source_store.latest_height()
    );
    let manifest = create_snapshot(&source_store, &snap_dir).expect("Failed to create snapshot");
    eprintln!(
        "Snapshot created: height={}, hash={}",
    ...
    let restored_manifest =
        restore_snapshot(&snap_dir, target_dir).expect("Failed to restore snapshot");
    ...
    let mut target_store =
        ChainStore::open(target_dir.to_str().unwrap()).expect("Failed to open target store");
    ...
    while current_gap_start <= manifest.snapshot_height {
        match download_missing_records(current_gap_start - 1, &peers) {
            Ok(records) => {
                ...
                eprintln!(
                    "Gap-fill: downloaded {} records from height {}",
                    records.len(),
                    records.first().unwrap().height
                );
                let new_h =
                    append_missing_records(&mut target_store, current_gap_start - 1, records)
            ...
            Err(e) => {
                eprintln!(
                    "Gap-fill: download failed at height {}: {}",
                    current_gap_start, e
                );
                break;
            }
    ...
    eprintln!(
        "Bootstrap complete: final_height={} store_records={} missing={}",
        final_height,
        target_store.len(),
        missing
    );
