...
fn n153_create_pool_and_swap() {
    ...
    let token_b = [2u8; 32];
    let pool_id = amm
        .create_pool(&mut reg, token_a, token_b, [10u8; 32])
        .unwrap();
...
fn n153_pool_evidence_root_deterministic() {
    ...
    let pool_id1 = amm1
        .create_pool(&mut reg1, [1u8; 32], [2u8; 32], [10u8; 32])
        .unwrap();
    let pool_id2 = amm2
        .create_pool(&mut reg2, [1u8; 32], [2u8; 32], [10u8; 32])
        .unwrap();
...
fn n153_swap_changes_evidence_root() {
    ...
    let mut amm = AmmEngine::new();
    let pool_id = amm
        .create_pool(&mut reg, [1u8; 32], [2u8; 32], [10u8; 32])
        .unwrap();
