...
fn byz_001_forged_proof_rejected() {
    ...
    let ctx = ExecutionContext {
        ...
        pre_state_root: [0u8; 32], authority: [2u8; 32],
    };
    ...
    let result = ConstitutionalRuntime::execute(
        &program, &ctx, &mut reg, &[], 10000, &mut hot, &mut archive,
    ).unwrap();
    ...
    assert!(!matches!(replay, amun_replay_verifier::replay_verifier::ReplayResult::Match { .. }));
}
...
fn byz_002_double_transfer_rejected() {
    ...
    assert!(registry.consume(&proof, make_id(99), 2, [0xdd; 32]).is_err());
}
...
fn byz_003_lineage_cycle_rejected() {
    ...
    reg.register_genesis(ResourceMetadata {
        resource_id: a, archetype: ResourceArchetype::Asset,
        state: ResourceState::Active, lineage: ResourceLineage::genesis(a),
        contract_id: [1u8; 32], owner: [2u8; 32],
    }).unwrap();
    let hash_a = ResourceRegistry::hash_resource(reg.get(&a).unwrap());
    reg.consume_and_derive(&a, ResourceMetadata {
        resource_id: b, archetype: ResourceArchetype::Asset,
    ...
    let c = make_id(3);
    assert!(reg.consume_and_derive(&b, ResourceMetadata {
        resource_id: c, archetype: ResourceArchetype::Asset,
        state: ResourceState::Active,
...
fn byz_004_version_regression_rejected() {
    ...
    reg.register_genesis(ResourceMetadata {
        resource_id: parent_id, archetype: ResourceArchetype::Asset,
        state: ResourceState::Active, lineage: ResourceLineage::genesis(parent_id),
        contract_id: [1u8; 32], owner: [2u8; 32],
    }).unwrap();
    let parent_hash = ResourceRegistry::hash_resource(reg.get(&parent_id).unwrap());
    let result = reg.consume_and_derive(&parent_id, ResourceMetadata {
...
fn byz_006_illegal_transformation_rejected() {
    ...
    let result = reg.consume_and_derive(&ev, ResourceMetadata {
        resource_id: make_id(2), archetype: ResourceArchetype::Asset,
        state: ResourceState::Active,
