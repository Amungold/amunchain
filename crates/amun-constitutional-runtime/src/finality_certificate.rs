...
impl ConstitutionalFinalityCertificate {
    pub fn issue(
        ...
        let proof_root =
            Self::compute_merkle_root(transitions.iter().map(|p| p.proof_hash).collect());
        let evidence_root = Self::compute_evidence_root(&transitions);
        let pccv_root = Self::compute_pccv_root(&transitions);
        let all_verified =
            block_result.block_valid && block_result.pccv_failed == 0 && block_result.rejected == 0;
        let pccv_results: Vec<String> =
            transitions.iter().map(|_| "VERIFIED".to_string()).collect();
        ...
        let mut cert = Self {
            ...
            pccv_root,
            qc_hash,
            previous_certificate_hash: [0u8; 32],
    ...
    pub fn compute_pccv_root(transitions: &[TransitionProof]) -> [u8; 32] {
        let hashes: Vec<[u8; 32]> = transitions
            .iter()
            .map(|p| {
                let mut hasher = blake3::Hasher::new();
...
mod tests {
    use super::*;
    use crate::block_validator::{BlockValidationResult, ConstitutionalBlockValidator};
    ...
    fn n52_issue_finality_certificate() {
        ...
        let programs: Vec<_> = (0..3)
            ...
            .collect();
        let block_result =
            ConstitutionalBlockValidator::validate_block(&programs, &mut registry, &[]).unwrap();
    ...
    fn n52_certificate_detects_tampering() {
        ...
        let mut cert =
            ConstitutionalFinalityCertificate::issue(&br, vec![], [0xaa; 32], 1, [0xbb; 32]);
