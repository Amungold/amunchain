...
impl CertificateChain {
    pub fn new(certificates: Vec<crate::finality_certificate::ConstitutionalFinalityCertificate>) -> Result<Self, String> {
        ...
        for i in 1..certificates.len() {
            ...
            if actual_prev != expected_prev {
                return Err(format!(
                    ...
                    i, hex::encode(expected_prev), hex::encode(actual_prev),
                ));
            }
        }
        let chain_root = Self::compute_chain_root(&certificates);
        let len = certificates.len(); Ok(Self { certificates, chain_root, chain_length: len })
    }

    pub fn verify_chain(&self) -> bool {
        if self.certificates.is_empty() { return false; }
        for cert in &self.certificates { if !cert.verify() { return false; } }
        for i in 1..self.certificates.len() {
            if self.certificates[i].previous_certificate_hash != self.certificates[i - 1].certificate_hash {
                return false;
            }
        }
    ...
    pub fn is_fully_constitutional(&self) -> bool {
        self.verify_chain() && self.certificates.iter().all(|c| c.is_constitutionally_valid())
    }
    ...
    fn compute_chain_root(certs: &[crate::finality_certificate::ConstitutionalFinalityCertificate]) -> [u8; 32] {
...
mod tests {
    ...
    fn n53_chain_deterministic_root() {
        let c1 = make_cert(1, [0x01; 32], [0xcc; 32], [0u8; 32]);
        let c2 = make_cert(2, [0x02; 32], [0xdd; 32], c1.certificate_hash);
        let r1 = CertificateChain::new(vec![c1.clone(), c2.clone()]).unwrap().chain_root;
    ...
    fn n53a_replaced_certificate_detected() {
        ...
        let fake = make_cert(2, [0xff; 32], [0xdd; 32], chain.certificates[0].certificate_hash);
