...
fn test_factor_isolation_recalibrated() {
    ...
    let protocol_a = ExperimentalProtocol {
        ...
        initial_recognition_density: 0.8, initial_treaty_density: 1.0,
    ...
    let last_a = SimulationRunner::run(&protocol_a).last().unwrap().mean_legitimacy;

    // Group B: Medium treaty density (0.5) — moderate foundation
    let protocol_b = ExperimentalProtocol {
        num_sovereigns: n, num_steps: steps,
        initial_recognition_density: 0.8, initial_treaty_density: 0.5,
        erosion_rate: erosion,
        formation_base_rate: 0.05, formation_legitimacy_factor: 0.10,
        formation_reciprocity_bias: 0.20,
        treaty_failure_rate: 0.0, horizon: steps,
    };
    let last_b = SimulationRunner::run(&protocol_b).last().unwrap().mean_legitimacy;
    ...
    let last_c = SimulationRunner::run(&protocol_c).last().unwrap().mean_legitimacy;
    ...
    assert!(last_a <= 1.0 && last_b <= 1.0 && last_c <= 1.0,
