use amun_constitutional_sim::{SimulationState, EffectivenessEngine};
...
fn test_effectiveness_bounds() {
    ...
    println!("Min E: {:.4}, Max E: {:.4}, Avg Ē: {:.4}", min_e, max_e, avg);
    println!("Claims: {}", state.effectiveness.len());

    assert!(max_e <= 1.0 + 1e-10, "E must be ≤ 1.0, got max={:.4}", max_e);
