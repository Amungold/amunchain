...
fn test_n100_validation_recalibrated() {
    let protocol = ExperimentalProtocol {
        num_sovereigns: 100, num_steps: 300,
        ...
        treaty_failure_rate: 0.0, horizon: 300,
    };
    let results = SimulationRunner::run(&protocol);
    let first = results.first().unwrap();
    let last = results.last().unwrap();
    println!("\n=== Recalibrated N=100 Validation ===");
    println!("Initial Ē: {:.4}, Final Ē: {:.4}", first.mean_legitimacy, last.mean_legitimacy);
    println!("Initial C_R: {:.4}, Final C_R: {:.4}", first.recognition_connectivity, last.recognition_connectivity);
    println!("Final D_L: {:.4}, Collapse Risk: {:.4}", last.divergence_from_start, last.collapse_risk);
    assert!(last.recognition_connectivity > 0.5, "Network must stay connected");
