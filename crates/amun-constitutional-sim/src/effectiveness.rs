use crate::state::{LegitimacyClaim, SimulationState};
...
impl EffectivenessEngine {
    ...
    fn witness_validity(_claim: &LegitimacyClaim) -> f64 {
        1.0
    }
    ...
    fn jurisdiction_match(claim: &LegitimacyClaim, state: &SimulationState) -> f64 {
        if claim.subject >= state.jurisdictions.len() {
            return 0.0;
        }
    ...
    fn epoch_validity(claim: &LegitimacyClaim) -> f64 {
        if claim.epoch_start <= claim.epoch_end {
            1.0
        } else {
            0.0
        }
    ...
    pub fn update_all(state: &mut SimulationState) {
        ...
        state.effectiveness = claims.iter().map(|c| Self::compute(c, state)).collect();
    }

    pub fn debug_claim(
        claim: &LegitimacyClaim,
