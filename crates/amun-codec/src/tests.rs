...
mod tests {
    ...
    fn test_u8_encode() {
        let mut buf = [0u8; 1];
        assert_eq!(255u8.encode(&mut buf).unwrap(), 1);
        assert_eq!(buf[0], 255);
    }
    ...
    fn test_u16_little_endian_freeze() {
        let mut buf = [0u8; 2];
        0x1122u16.encode(&mut buf).unwrap();
    ...
    fn test_u64_little_endian_freeze() {
        let mut buf = [0u8; 8];
        0x1122334455667788u64.encode(&mut buf).unwrap();
    ...
    fn test_u8_roundtrip() {
        ...
        let (d, len) = u8::decode(&buf).unwrap();
        assert_eq!(len, 1);
        assert_eq!(d, v);
    }
    ...
    fn test_u16_roundtrip() {
        ...
        let mut buf = [0u8; 2];
        v.encode(&mut buf).unwrap();
        let (d, len) = u16::decode(&buf).unwrap();
        assert_eq!(len, 2);
        assert_eq!(d, v);
    }
    ...
    fn test_u32_roundtrip() {
        ...
        let mut buf = [0u8; 4];
        v.encode(&mut buf).unwrap();
        let (d, len) = u32::decode(&buf).unwrap();
        assert_eq!(len, 4);
        assert_eq!(d, v);
    }
    ...
    fn test_u64_roundtrip() {
        ...
        let mut buf = [0u8; 8];
        v.encode(&mut buf).unwrap();
        let (d, len) = u64::decode(&buf).unwrap();
        assert_eq!(len, 8);
        assert_eq!(d, v);
    }
    ...
    fn test_u128_roundtrip() {
        ...
        let mut buf = [0u8; 16];
        v.encode(&mut buf).unwrap();
        let (d, len) = u128::decode(&buf).unwrap();
        assert_eq!(len, 16);
        assert_eq!(d, v);
    }
    ...
    fn test_bytes32_roundtrip() {
        ...
        let mut buf = [0u8; 32];
        v.encode(&mut buf).unwrap();
        let (d, len) = <[u8; 32]>::decode(&buf).unwrap();
        assert_eq!(len, 32);
        assert_eq!(d, v);
    }
    ...
    fn test_decode_exact_exact_fits() {
        let mut buf = [0u8; 8];
        42u64.encode(&mut buf).unwrap();
        assert_eq!(u64::decode_exact(&buf).unwrap(), 42);
    }
    ...
    fn test_epoch_roundtrip() {
        ...
        let (d, _) = Epoch::decode(&buf).unwrap();
        assert_eq!(d, e);
    }
    ...
    fn test_round_roundtrip() {
        ...
        let (d, _) = Round::decode(&buf).unwrap();
        assert_eq!(d, r);
    }
    ...
    fn test_hash32_roundtrip() {
        ...
        let (d, _) = PublicHash32::decode(&buf).unwrap();
        assert_eq!(d, h);
    }
    ...
    fn test_validator_id_roundtrip() {
        ...
        let mut buf = [0u8; 32];
        v.encode(&mut buf).unwrap();
        let (d, _) = ValidatorId::decode(&buf).unwrap();
        assert_eq!(d, v);
    }
