...
fn n154_loan_creation_and_repayment() {
    ...
    let (loan_id, _collateral_id) = engine
        .create_loan(
            &mut amun_resource_core::ResourceRegistry::new(100),
            borrower,
            1000,
            500,
            2000,
...
fn n154_lending_root_deterministic() {
    ...
    let (_id2, _) = engine2
        ...
        .unwrap();
    assert_eq!(
        engine1.compute_lending_root(),
        engine2.compute_lending_root()
    );
}
