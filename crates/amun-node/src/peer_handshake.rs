...
impl HandshakeMessage {
    ...
    pub fn verify(&self, our_genesis_hash: &[u8; 32]) -> Result<(), String> {
        ...
        if !self.certificate.verify(&self.public_key) {
...
mod tests {
    ...
    fn n22_5_valid_handshake_accepted() {
        ...
        assert!(handshake.verify(&genesis_hash).is_ok());
    }
    ...
    fn n22_5_genesis_mismatch_rejected() {
        ...
        assert!(handshake.verify(&wrong_genesis).is_err());
    }
    ...
    fn n22_5_tampered_certificate_rejected() {
        ...
        assert!(handshake.verify(&genesis_hash).is_err());
    }

    #[test]
    fn n22_5_authenticated_peer_creation() {
        let (handshake, genesis_hash) = create_test_handshake();
        handshake.verify(&genesis_hash).unwrap();
