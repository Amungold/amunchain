...
fn main() {
    ...
    println!("Live validators height: {}, root: {}", current_height, hex::encode(live_root));
    ...
    match result {
        RejoinResult::Rejoined { height, resources_imported } => {
            println!("Validator 2 rejoined successfully at height {} with {} resources",
    ...
    match result4 {
        RejoinResult::Rejoined { height, resources_imported } => {
            println!("Validator 4 rejoined successfully at height {} with {} resources",
                     height, resources_imported);
        }
        RejoinResult::Failed { reason } => {
