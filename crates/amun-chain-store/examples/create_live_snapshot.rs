...
fn main() {
    ...
    let store_dir = args.get(1).map(|s| s.as_str()).unwrap_or("/tmp/amun-test-validator-0");
    let snap_dir = args.get(2).map(|s| s.as_str()).unwrap_or("/tmp/amun-test-validator-0/snapshot");
