...
impl ChainStore {
    pub fn open(data_dir: &str) -> Result<Self, StoreError> {
        ...
        if chain_file.exists() && index_file.exists() {
            ...
            Ok(Self { data_dir, records: Vec::new(), offsets: Vec::new() })
        }
    }
    ...
    fn load(data_dir: &Path, chain_file: &Path, index_file: &Path) -> Result<Self, StoreError> {
        ...
        Ok(Self { data_dir: data_dir.to_path_buf(), records, offsets })
    }
    ...
    pub fn load_height(&self, height: u64) -> Option<&FinalizedChainRecord> { self.records.get(height as usize) }
