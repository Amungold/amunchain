...
fn main() {
    ...
    let ports = [base_port, base_port + 1, base_port + 2, base_port + 3];
    let v2 = LiveValidator::new(ValidatorConfig::test_cluster(2, &ports).with_quorum(quorum));
    let v3 = LiveValidator::new(ValidatorConfig::test_cluster(3, &ports).with_quorum(quorum));
    v2.start().unwrap();
    v3.start().unwrap();
    validators[2] = v2;
    validators[3] = v3;
