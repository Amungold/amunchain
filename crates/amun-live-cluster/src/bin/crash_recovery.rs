...
fn main() {
    ...
    for _ in 0..20 {
        thread::sleep(Duration::from_secs(1));
        let t = start.elapsed().as_secs();
        let h: Vec<u64> = validators[0..3].iter().map(|v| v.store.lock().unwrap().latest_height()).collect();
        println!("t={:3}s heights=[{}, {}, {}, --]", t, h[0], h[1], h[2]);
    }

    println!("=== Phase 3: Restarting validator 3 ===");
    let new_v3 = LiveValidator::new(
    ...
    println!("  Recovery verdict:    {}", if spread <= 2 { "PASS" } else { "PARTIAL" });
    println!("============================================");
}
