...
fn main() {
    ...
    println!("\nPhase 2: Initialize validator_status, set IDs, suspend validator 2...");
    {
        ...
        eng.validator_status
            ...
            .set_status(
                target,
                ValidatorStatus::Suspended { until_height: eng.current_height + 200 },
            );

        let active_after = eng.active_validator_count();
        println!("  Active after:  {}", active_after);
        println!("  Exclusion:     {}", if active_before == 4 && active_after == 3 { "PASS" } else { "FAIL" });
    }
    ...
    let final_active = validators[0].engine.lock().unwrap().active_validator_count();
    ...
    println!("  Network:         {}", if spread <= 2 { "PASS" } else { "FAIL" });
    println!("  Quorum Exclusion: {}", if final_active == 3 { "PASS" } else { "PARTIAL" });
    println!("============================================");
}
