...
fn n141_reject_unregistered_token() {
    ...
    let result = RightsEnforcementEngine::validate_transfer(
        &reg, &bridge, &gov, &[1u8; 32], &[10u8; 32], &[20u8; 32], 100
    );
...
fn n141_reject_non_owner_seller() {
    ...
    let gov = GovernanceLedger::new();
    let result = RightsEnforcementEngine::validate_transfer(
        &reg, &bridge, &gov, &token, &[99u8; 32], &[20u8; 32], 100
    );
    assert!(!result.allowed);
    assert_eq!(result.reason, Some("Seller is not constitutional owner".into()));
}
...
fn n141_reject_bridge_locked_transfer() {
    ...
    let result = RightsEnforcementEngine::validate_transfer(
        &reg, &bridge, &gov, &token, &[10u8; 32], &[30u8; 32], 200
    );
    assert!(!result.allowed);
    assert_eq!(result.reason, Some("Token is locked in cross-chain bridge".into()));
}
...
fn n141_allow_valid_transfer_with_royalty() {
    ...
    gov.set_rights(GovernanceRight {
        token_id: token, owner: seller, can_propose: true, can_veto: false, voting_power: 10,
    });
    reg.register(NftConstitutionalRecord {
        token_id: token, owner: seller, collection_id: None, creator: [30u8; 32],
        mining_origin: None,
    ...
    let result = RightsEnforcementEngine::validate_transfer(
        &reg, &bridge, &gov, &token, &seller, &[20u8; 32], 1000
    );
    assert!(result.allowed);
