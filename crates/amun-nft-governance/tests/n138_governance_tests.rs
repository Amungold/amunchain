...
fn n138_multiple_tokens_independent_rights() {
    ...
    let o1 = [10u8; 32];
    let o2 = [20u8; 32];
    ledger.set_rights(GovernanceRight { token_id: t1, owner: o1, can_propose: true, can_veto: false, voting_power: 10 });
    ledger.set_rights(GovernanceRight { token_id: t2, owner: o2, can_propose: false, can_veto: true, voting_power: 20 });
...
fn n138_deterministic_governance_root() {
    ...
    let right = GovernanceRight { token_id: [5u8; 32], owner: [50u8; 32], can_propose: true, can_veto: true, voting_power: 1000 };
...
fn n138_revoked_changes_root() {
    ...
    let mut l2 = GovernanceLedger::new();
    let token = [7u8; 32];
    let right = GovernanceRight { token_id: token, owner: [70u8; 32], can_propose: true, can_veto: false, voting_power: 5 };
    l1.set_rights(right.clone());
    l2.set_rights(right);
