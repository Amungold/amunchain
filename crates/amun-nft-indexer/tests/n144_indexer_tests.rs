...
fn n144_index_and_query_nft() {
    ...
    reg.register(NftConstitutionalRecord {
        token_id: token, owner: [10u8; 32], collection_id: Some([5u8; 32]), creator: [30u8; 32],
        mining_origin: Some("Validator".into()),
        royalty_policy: None, governance_right: None, bridge_lock: None,
    });
...
fn n144_query_by_owner() {
    ...
    for i in 0..3u8 {
        reg.register(NftConstitutionalRecord {
            token_id: [i; 32], owner, collection_id: None, creator: [30u8; 32],
...
fn n144_index_events_and_query() {
    let mut indexer = NftIndexer::new();
    let events = vec![
        MarketplaceEvent::ListingCreated { token_id: ResourceId([1u8; 32]), seller: [10u8; 32], price: 100 },
        MarketplaceEvent::SaleCompleted { token_id: ResourceId([1u8; 32]), seller: [10u8; 32], buyer: [20u8; 32], price: 100 },
    ];
