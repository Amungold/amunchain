...
fn n156_lock_nft_and_borrow() {
    ...
    let loan_id = engine.borrow_against_nft(&mut reg, token_id, owner, 500, 100, 1).unwrap();
...
fn n156_repay_and_unlock() {
    ...
    let mut engine = NftCollateralEngine::new();
    engine.lock_nft(&reg, token_id, &owner).unwrap();
    let loan_id = engine.borrow_against_nft(&mut reg, token_id, owner, 200, 0, 1).unwrap();
    engine.repay_and_unlock(&mut reg, &loan_id, token_id, 200).unwrap();
    assert!(!engine.is_locked(&token_id));
}
...
fn n156_liquidation_removes_lock() {
    ...
    let mut engine = NftCollateralEngine::new();
    engine.lock_nft(&reg, token_id, &owner).unwrap();
    let loan_id = engine.borrow_against_nft(&mut reg, token_id, owner, 500, 500, 1).unwrap();
    let health = engine.lending.get_health_factor(&loan_id.0, 2_000_000);
    assert!(InterestModel::is_liquidatable(health));
    engine.liquidate(&loan_id, token_id, [99u8; 32], 2_000_000).unwrap();
    assert!(!engine.is_locked(&token_id));
}
...
fn n156_evidence_root_deterministic() {
    ...
    reg1.register_genesis(ResourceMetadata {
        resource_id: token_id1, archetype: ResourceArchetype::NFTAsset,
        state: ResourceState::Active, lineage: ResourceLineage::genesis(token_id1),
        contract_id: [0u8; 32], owner,
    }).unwrap();
    reg2.register_genesis(ResourceMetadata {
        resource_id: token_id2, archetype: ResourceArchetype::NFTAsset,
        state: ResourceState::Active, lineage: ResourceLineage::genesis(token_id2),
        contract_id: [0u8; 32], owner,
    }).unwrap();
    ...
    engine1.borrow_against_nft(&mut reg1, token_id1, owner, 300, 200, 1).unwrap();
    engine2.borrow_against_nft(&mut reg2, token_id2, owner, 300, 200, 1).unwrap();
    assert_eq!(engine1.compute_evidence_root(), engine2.compute_evidence_root());
}
