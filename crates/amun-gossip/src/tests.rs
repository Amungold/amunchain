...
fn test_topic_roundtrip() {
    for topic in &[
        ...
        let b = topic.as_byte();
        let decoded = Topic::from_byte(b).expect("test invariant");
...
fn test_broadcaster_rejects_duplicate() {
    ...
    let count1 = b
        .broadcast(Topic::Blocks, payload, 10)
...
fn test_receiver_counts_unique() {
    ...
    assert!(r.receive(Topic::Votes, b"vote1").expect("test invariant"));
    assert_eq!(r.received_count(), 1);
    assert!(!r.receive(Topic::Votes, b"vote1").expect("test invariant"));
    assert_eq!(r.received_count(), 1);
    assert!(r.receive(Topic::Votes, b"vote2").expect("test invariant"));
