...
mod tests {
    use amun_consensus_execution::{
        LeaderSchedule, VoteAggregator, CommitRule,
        BlockDAG, BlockNode, ForkChoice, Pacemaker, QCStore,
    };
    ...
    fn create_test_qc(block_hash: [u8; 32], round: u64, position: ChainPosition, parent_hash: [u8; 32]) -> QuorumCertificate {
        let votes = (1..=4)
            .map(|id| ConsensusVote::new(id, position, round, ConsensusPhase::Prevote, Some(block_hash), [id as u8; 64], 25))
            .collect();
        QuorumCertificate::new(position, round, block_hash, parent_hash, votes)
    }

    fn test_validator_set() -> ValidatorSet {
        ValidatorSet::new(1, vec![
    ...
    fn test_vote_aggregator_partitions_by_block_hash() {
        let set = ValidatorSet::new(1, vec![
            ValidatorInfo { id: 1, public_key: [1u8; 32], stake: 40 },
            ValidatorInfo { id: 2, public_key: [2u8; 32], stake: 30 },
            ValidatorInfo { id: 3, public_key: [3u8; 32], stake: 30 },
        ]).unwrap();
        let mut agg = VoteAggregator::new(set);
        let pos = ChainPosition::new(0, 1);
        let va = ConsensusVote::new(1, pos, 0, ConsensusPhase::Prevote, Some([0xAA; 32]), [1u8; 64], 40);
        assert!(agg.add_vote(va).unwrap().is_none());
        let vb = ConsensusVote::new(2, pos, 0, ConsensusPhase::Prevote, Some([0xBB; 32]), [2u8; 64], 30);
        assert!(agg.add_vote(vb).unwrap().is_none());
        let va2 = ConsensusVote::new(3, pos, 0, ConsensusPhase::Prevote, Some([0xAA; 32]), [3u8; 64], 30);
    ...
    fn test_equivocation_detection() {
        ...
        let mut agg = VoteAggregator::new(set);
        let pos = ChainPosition::new(0, 1);
        let v1 = ConsensusVote::new(1, pos, 0, ConsensusPhase::Prevote, Some([0xAA; 32]), [1u8; 64], 25);
        assert!(agg.add_vote(v1).is_ok());
        let v1_equiv = ConsensusVote::new(1, pos, 0, ConsensusPhase::Prevote, Some([0xBB; 32]), [1u8; 64], 25);
    ...
    fn test_e2e_protocol_trace_proposal_to_commit() {
        ...
        for vid in 1..=4 {
            let vote = ConsensusVote::new(vid, pos1, 0, ConsensusPhase::Precommit, Some(block1), [vid as u8; 64], 25);
            if let Ok(Some(qc)) = agg.add_vote(vote) { precommit_qc = Some(qc); }
        }
        assert!(precommit_qc.is_some());
        assert!(commit_rule.try_commit_2chain(&precommit_qc.unwrap()).is_none());
        let mut agg2 = VoteAggregator::new(set);
        let mut prevote_qc2 = None;
        for vid in 1..=4 {
            let vote = ConsensusVote::new(vid, pos2, 1, ConsensusPhase::Prevote, Some(block2), [vid as u8; 64], 25);
            if let Ok(Some(qc)) = agg2.add_vote(vote) { prevote_qc2 = Some(qc); }
        }
    ...
    fn test_block_dag_ancestry() {
        ...
        let block1 = [0x01; 32];
        let block2 = [0x02; 32];
        dag.add_block(BlockNode::new(block1, Some(genesis), ChainPosition::new(0, 1), 1, None, block1)).unwrap();
        dag.add_block(BlockNode::new(block2, Some(block1), ChainPosition::new(0, 2), 2, None, block2)).unwrap();
    ...
    fn test_block_dag_depth_bound() {
        ...
        dag.add_block(BlockNode::new(far_child, Some(genesis), ChainPosition::new(0, 1), 1, None, far_child)).unwrap();
    ...
    fn test_children_index() {
        ...
        let child1 = [0x01; 32];
        let child2 = [0x02; 32];
        dag.add_block(BlockNode::new(child1, Some(genesis), ChainPosition::new(0, 1), 1, None, child1)).unwrap();
        dag.add_block(BlockNode::new(child2, Some(genesis), ChainPosition::new(0, 1), 1, None, child2)).unwrap();
        let children = dag.get_children(&genesis);
        assert_eq!(children.len(), 2);
        let grandchild = [0x03; 32];
        dag.add_block(BlockNode::new(grandchild, Some(child1), ChainPosition::new(0, 2), 2, None, grandchild)).unwrap();
    ...
    fn test_fork_choice_canonical_tip() {
        ...
        dag.add_block(BlockNode::new(block_a, Some(genesis), pos1, 1, Some(qc_a1.clone()), block_a)).unwrap();
        dag.add_block(BlockNode::new(a2, Some(block_a), pos2, 2, Some(qc_a2.clone()), a2)).unwrap();
    ...
    fn test_safe_node_predicate() {
        ...
        dag.add_block(BlockNode::new(block_a, Some(genesis), pos1, 1, Some(qc_a.clone()), block_a)).unwrap();
        ...
        let unsafe_block = BlockNode::new([0xCC; 32], Some(block_a), pos2, 2, Some(qc_wrong), [0xCC; 32]);
    ...
    fn test_lock_monotonicity() {
        ...
        dag.add_block(BlockNode::new(block1, Some(genesis), pos1, 1, Some(qc1.clone()), block1)).unwrap();
        fc.update_qc(qc1, &dag);
        dag.add_block(BlockNode::new(block2, Some(block1), pos2, 2, Some(qc2.clone()), block2)).unwrap();
        fc.update_qc(qc2, &dag);
        dag.add_block(BlockNode::new(block3, Some(block2), pos3, 3, Some(qc3.clone()), block3)).unwrap();
    ...
    fn test_finalized_pruning_preserves_canonical_spine() {
        ...
        for i in 1..=5 {
            let hash = [i as u8; 32];
            dag.add_block(BlockNode::new(hash, Some(prev), ChainPosition::new(0, i), i, None, hash)).unwrap();
            prev = hash;
        }
        let fork = [0xF2; 32];
        dag.add_block(BlockNode::new(fork, Some([1u8; 32]), ChainPosition::new(0, 2), 2, None, fork)).unwrap();
    ...
    fn test_qc_store_pruning() {
        let mut store = QCStore::new();
        for r in 0..10 {
            store.insert(create_test_qc([r as u8; 32], r, ChainPosition::new(0, r + 1), [0x00; 32]));
        }
        store.prune_below(5);
    ...
    fn test_network_partition_fork_choice_reconciliation() {
        ...
        merged_dag.add_block(BlockNode::new(chain_a1, Some(genesis), pos1, 1, Some(create_test_qc(chain_a1, 1, pos1, genesis)), chain_a1)).unwrap();
        merged_dag.add_block(BlockNode::new(chain_a2, Some(chain_a1), pos2, 2, Some(create_test_qc(chain_a2, 2, pos2, chain_a1)), chain_a2)).unwrap();
        merged_dag.add_block(BlockNode::new(chain_b1, Some(genesis), pos1, 1, Some(create_test_qc(chain_b1, 1, pos1, genesis)), chain_b1)).unwrap();
        merged_dag.add_block(BlockNode::new(chain_b2, Some(chain_b1), pos2, 2, Some(create_test_qc(chain_b2, 2, pos2, chain_b1)), chain_b2)).unwrap();
